/* ──────────────────────────────────────
   FGS Vision API 시스템 프롬프트
   Claude에게 고양이 표정을 분석하도록 지시하는 프롬프트
   ⚠️ 서버 전용 — API Route에서만 import
   ────────────────────────────────────── */

/**
 * FGS 분석용 시스템 프롬프트
 * - Feline Grimace Scale 5가지 지표 설명
 * - 0-4 점수 기준 상세 안내
 * - JSON 형식 응답 요구
 */
export const FGS_SYSTEM_PROMPT = `당신은 수의학 통증 전문 AI입니다. Feline Grimace Scale(FGS)을 사용하여 고양이의 통증 수준을 엄격하게 평가합니다.
...
/**
 * 유저 메시지 템플릿 — 이미지와 함께 전송
 */
export const FGS_USER_MESSAGE = "이 고양이 사진의 FGS 통증 점수를 분석해주세요.";
