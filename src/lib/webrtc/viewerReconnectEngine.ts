...
export class ViewerReconnectEngine {
  ...
  private async checkKeepalive(): Promise<void> {
    ...
    try {
      ...
      let totalBytes = 0;
      /*
       * hasActiveTrack — 수신 track 이 살아있는지 확인.
       * track 이 muted/ended 상태면 bytesReceived 가 동결되어
       * 정상 상황을 keepalive_dead 로 오판정하는 false positive 가 발생한다.
       * remoteSource=true 이고 ended=false 인 track 이 1개 이상 있을 때만 stale 로 카운트.
       */
      let hasActiveTrack = false;
      stats.forEach((r) => {
        if (r.type === "inbound-rtp" && typeof r.bytesReceived === "number") totalBytes += r.bytesReceived;
        /* track 리포트 — Chromium 기준 remoteSource/ended 필드 사용 */
        const tr = r as unknown as { type: string; remoteSource?: boolean; ended?: boolean };
        if (tr.type === "track" && tr.remoteSource === true && tr.ended === false) {
          hasActiveTrack = true;
        }
      });
      /*
       * 활성 track 이 없으면 stale 카운트 스킵 — muted/ended 상태의 동결을
       * keepalive_dead 로 오판정하는 false positive 방지.
       * 단, lastBytesReceived 는 계속 갱신해 복구 시점을 정확히 잡는다.
       */
      if (!hasActiveTrack) {
        this.staleCount = 0;
