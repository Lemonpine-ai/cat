...
export function mapRowToPost(r: Record<string, unknown>): CommunityPost {
  ...
  // profiles 테이블의 실제 컬럼명은 display_name입니다 (nickname 아님)
  ...
  return {
    ...
    // 이미지 URL — 없으면 null로 처리합니다.
    image_url: (r.image_url as string) ?? null,
    // 건강/질병 서브 태그 — health 카테고리에서만 사용
    health_tag: (r.health_tag as string) ?? null,
    like_count: r.like_count as number,
    comment_count: r.comment_count as number,
    created_at: r.created_at as string,
    updated_at: r.updated_at as string,
    // 프로필에서 표시 이름을 가져오되, 없으면 null로 둡니다.
...
export function mapRowToComment(r: Record<string, unknown>): CommunityComment {
  ...
  // profiles 테이블의 실제 컬럼명은 display_name입니다
  const profile = r.profiles as { display_name?: string } | null;
