...
import { useBroadcasterPeerMap } from "@/../staging/hooks/useBroadcasterPeerMap";
...
/**
 * 남는 폰에서 실행하는 WebRTC 방송 클라이언트 (Multi-Viewer 버전).
 ...
 */
export function CameraBroadcastClientMulti() {
  ...
  const signaling = useBroadcasterPeerMap({
    deviceToken: deviceIdentity?.deviceToken ?? null,
   ...
   * replaceVideoTrack 은 useBroadcasterPeerMap 내부에서 useCallback([], []) 로
