...
const BROADCAST_CODE_VERSION = "v4-landscape-lock";
...
export function CameraBroadcastClient() {
  ...
  /* 가로모드 강제 고정 — requestLandscapeLock()은 사용자 제스처(카메라 켜기)에서 호출 */
  ...
  async function acquireCamera() {
    ...
    /* fullscreen + orientation lock — 사용자 제스처 컨텍스트에서 호출 */
  ...
  async function startBroadcast(opts?: { forceRelay?: boolean }) {
    ...
    try {
      ...
      /*
       * signaling 타임아웃 — 25초 이내에 answer 미수신 시 세션 재생성.
       * 2대 동시 연결 시 stagger 여유를 위해 15초 → 25초로 완화.
      ...
      signalingTimeoutRef.current = setTimeout(() => {
        ...
        if (pc && pc.connectionState !== "connected") {
          console.log("[broadcaster] signaling 타임아웃 25초 — 세션 재생성");
          void (async () => {
            await cleanupPeerResourcesOnly(true);
            void startBroadcast();
          })();
  ...
  return (
    <div className={styles.page}>
      {/* 세로 모드 안내 — idle 상태에서는 숨김 (카메라 켜기 버튼 가림 방지) */}
      <PortraitOverlay visible={isPortrait && broadcastPhase !== "idle"} />
