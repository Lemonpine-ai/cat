...
| R4-5 | 68070db | 마무리 — canvas mock / baseline 갱신 / sample_count empty / docstring 정정 |

### fix R5 (3 commit, 본 PR)
| R5-1 | 951c78d | 헌법 base 회복 + commit hygiene — master(5824498) merge / debug-file 제거 / weight rollback 등록 |
| R5-2 | 4d8a8e1 | R3 단순화 + R7 보안 — useCatSubmitFlow / useCatPhotoUpload / CatRadioGroup / CatTextArea / size 가드 / HEIC 제거 / PII path |
| R5-3 | (본 commit) | R8 운영 + 문서 — atomic deploy 5a~5e smoke / CI lockfile / PR 템플릿 / baseline 갱신 |

## 검증 게이트 (각 commit 후)

- `npx tsc --noEmit` = exit 0 (cat-identity 영역 새 에러 0) ✅
- `npx vitest run` = 28 files / 200 passed ✅ (fix R5 후 — 신규 21 케이스 추가, 회귀 0)
- `npm run verify:routes` = 1 라우트 OK ✅
- `npx next build` = cat-identity 컴파일 통과 ✅ (community/write baseline 실패는 master env var 부재 — 무관)
